		...
		editAccount: function($editAccountFormData) {
			...
			.then(function(response) {
				...
				if(response.data.status === appValue.API_SUCCESS){
					$ionicPopup.alert({
						title: $rootScope.appLanguage.MESSAGE_TEXT,
						template: $rootScope.appLanguage.MESSAGE_TEXT_OK
					});
					return true;
				}
				else {
			...
			return $http({
				...
				.then(function(response) {
					...
					if (response.data.status === appValue.API_SUCCESS){
						$ionicPopup.alert({
							title: $rootScope.appLanguage.MESSAGE_TEXT,
							template: 'Your comment2 is awaiting approval'
						});
						return true;
					} else {
						$ionicPopup.alert({
    ...
    return {
        WC: function(){
            var Woocommerce = new WooCommerceAPI.WooCommerceAPI({
                url: 'http://anaketdelivery.com',
                consumerKey: 'ck_91b2cfc1bf9b050483fcdccfb2a4b092b939b675',
                consumerSecret: 'cs_94f94240c192cb6ef2c3db147b92334d147bee00'
            });
            return Woocommerce;
        }
	}
