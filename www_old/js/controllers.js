angular.module('starter.controllers', ['ngCordova', 'ionic-ratings'])
...
.controller('ProductCtrl', function($scope, $ionicModal, $sce, $timeout, $ionicPopup, $rootScope, $ionicSlideBoxDelegate, appConfig, CartService, WishlistService, ReviewsService, UserService, AnimationService, $state) {
	...
	$scope.addProductToCart = function() {
		...
		if($scope.product.type === 'variable') {
			if($scope.productFormData.variation_id === -1){
				$ionicPopup.alert({
			     	title: $rootScope.appLanguage.MESSAGE_TEXT,
			     	template: 'Por favor, seleccione algunas opciones del producto antes de añadir este producto a su cesta.'
			   	});
				return false;
			}
			productId = $scope.productFormData.variation_id;
                ...
                if(info.price === "") {
                        $ionicPopup.alert({
                        title: $rootScope.appLanguage.MESSAGE_TEXT,
                        template: 'Lo sentimos, este producto no está disponible. Por favor, elija una combinación diferente.'
                        });
                        return false;
                }
	...
	$scope.submitProductReview = function(reviewSubmitForm) {
		...
		if(rating === 0){
			$ionicPopup.alert({
				title: $rootScope.appLanguage.MESSAGE_TEXT,
				template: 'Por favor seleccione una calificiacion'
			});
			return false;
		}
		...
		$ionicPopup.alert({
			title: $rootScope.appLanguage.MESSAGE_TEXT,
			template: 'Muchas Gracias por su comentario!'
		});
...
/*
.controller('CompleteCtrl', ['$http', controladorPrincipal]);
...
//Controller Cambio Estado Orden

.controller('OrderStateCtrl', function($scope, WC){
...
.controller('ScannerCtrl', function($scope, $http, $rootScope, $ionicPopup, $cordovaBarcodeScanner, $ionicPlatform) {
	var vm = this;
		...
		$ionicPlatform.ready(function() {
			$cordovaBarcodeScanner
				...
				.then(function(result) {
					...
					vm.scanResults = "Codigo Capturado: " + result.text + "\n" +
	...
	$scope.showPopup = function() {
		...
		$ionicPopup.show({
		  ...
		  title: 'Contactar con Anaket',
		  subTitle: 'Llamanos al 809-123-4567 o al Email: info@anaketdelivery.com',
		 // scope: $scope,
		  buttons: [
		/* 	{ text: 'Cancel', onTap: function(e) { return true; } },*/
		  ...
		  $scope.showPopupSI = function() {
			$scope.data = {}
	
			$ionicPopup.show({
			  templateUrl: 'popup-template.html',
			  title: 'Pedido Completado!',
			  subTitle: 'Su pedido fue completado con Exito, gracias por confiar en nosotros, visita www.AnaketDelivery.com para conocer mas!',
			  scope: $scope,
			  buttons: [
			/* 	{ text: 'Cancel', onTap: function(e) { return true; } },*/
				{
